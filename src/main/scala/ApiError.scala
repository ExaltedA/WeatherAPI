...
object ApiError {
  ...
  val emptyTitleField: ApiError = new ApiError(StatusCodes.BadRequest, "City name field must not be empty.")

  def cityNotFound(name: String): ApiError =
    new ApiError(StatusCodes.NotFound, s"City with name $name could not be found.")
}